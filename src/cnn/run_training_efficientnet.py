# src/cnn/run_training_efficientnet.py
...
# --- 프로젝트 경로 설정 ---
# 현재 파일 위치: /src/cnn/run_training_efficientnet.py
# 프로젝트 루트: /
...
from src.cnn.models_efficientnet import MultiHeadEfficientNetB0  # EfficientNet 모델 임포트
from src.cnn.losses import multihead_ce_loss
from src.cnn.train import get_device, train_one_epoch

# --- 유틸리티 클래스 및 함수 (노트북에서 가져옴) ---
...
def extract_images_and_labels(batch: Any) -> Tuple[torch.Tensor, torch.Tensor]:
    # (노트북의 `extract_images_and_labels` 함수 내용과 동일)
...
def main():
    ...
    # --- 주요 설정 ---
    EXPERIMENT_NAME = "E5_efficientnet_b0_baseline"
    BATCH_SIZE = 16  # VRAM 부족 시 16 또는 8로 줄여보세요
    NUM_EPOCHS = 20  # Early Stopping으로 조기 종료될 수 있음
    LR = 1e-4
    USE_SUBSET = False  # True로 바꾸면 100개 샘플로 오버핏 테스트
    ...
    # --- 모델, 옵티마이저, 스케줄러 ---
    device = get_device()
    model = MultiHeadEfficientNetB0()  # 모델 변경
    model.to(device)
